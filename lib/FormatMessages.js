...
class FormatMessages {
  ...
  handleDone (isDevelopment, isProduction, isTTY, hasWarnings, hasErrors, hasDevServer, shouldNotify, messages, compiler, stats) {
    ...
    if (isDevelopment || isProduction) {
      ...
      if (hasWarnings && !hasErrors) {
        ...
        // The custom formatter adds 2 spaces between phrases:
        // message[0] === '  '
        // message[1] === 'Line 6:'
        // message[2] === 'Strings must use singlequote'
        message = stripAnsi(messages.warnings[0].split('\n')[1]).split('  ');

        if (this.hasNotifications) {
          notifier.notify({
            title: TITLE,
            message: `Warning on ${message[1].toLowerCase()} ${message[2]}`,
            contentImage: isMac ? ICON : null,
            icon: isLinux || isWindows ? ICON : null
          });
        }
   ...
   */
  handleExit (isTTY, code, signal) {
    if (isTTY) {
      this.ora.stop();
    }

    // Don't clear the console after a build unless you CTRL-C while compiling
    if (process.env.NODE_ENV !== 'production' && signal !== 'SIGINT' && isTTY) {
      this.clearConsole();
    }
  }
}
